...
def test_001_auth_domain_create():
    auth_domain_create(handle, name="test_auth_dom", refresh_period="800")
...
def test_002_auth_domain_modify():
    ...
    mo = auth_domain_realm_configure(
        handle, domain_name="test_auth_dom", realm="ldap")
    assert_equal(mo.realm, "ldap")


def test_003_native_auth_configure():
    native_authentication_configure(
        handle, def_role_policy="assign-default-role", con_login="local")
    mo = native_authentication_configure(handle, def_role_policy="no-login")
    assert_equal(mo.def_role_policy, "no-login")


def test_004_native_auth_default():
    mo = native_authentication_default(handle, realm="radius")
    assert_equal(mo.realm, "radius")
    native_authentication_default(handle, realm="local")


def test_005_native_auth_console():
    mo = native_authentication_console(handle, realm="local")
